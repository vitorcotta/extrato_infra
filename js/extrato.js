  ...
  async function loadExtratoData() {
    try {
      ...
      document.getElementById('headerInfo').innerHTML = `
        <h2>${data.division}</h2>
        <h3>Mês: ${data.registroA.month}</h3>
      `;
      
      // Disponibilidade
      document.getElementById('availability-general').textContent = `${data.registroA['availability.availability']} %`;
      ...
      systemsCount.textContent = data.registroA['availability.systems'];
      systemsCount.appendChild(createVariationElement(data.registroA['pct_change_availability_systems']));
      
      // Eventos
      const eventsCount = document.getElementById('events-count');
      eventsCount.textContent = data.registroA['availability.events'];
      ...
      unavailableTime.textContent = `${data.registroA['availability.time']} min`;
      ...
      vmCount.textContent = `VMs Ativas: ${data.registroA['vm.count']}`;
      vmCount.appendChild(createVariationElement(data.registroA['pct_change_vm_count']));
      
      document.getElementById('vm-vcpu').textContent = `vCPU: ${data.registroA['vm.vcpu']}`;
      document.getElementById('vm-vcpu').appendChild(createVariationElement(data.registroA['pct_change_vm_vcpu']));
      
      document.getElementById('vm-memory').textContent = `Memória: ${data.registroA['vm.memory']}`;
      document.getElementById('vm-memory').appendChild(createVariationElement(data.registroA['pct_change_vm_memory']));
      
      document.getElementById('vm-storage').textContent = `Storage: ${data.registroA['vm.storage']}`;
      ...
      podCount.textContent = `PODs: ${data.registroA['pod.pod']}`;
      podCount.appendChild(createVariationElement(data.registroA['pct_change_pod_pod']));
      
      document.getElementById('pod-vcpu').textContent = `vCPU: ${data.registroA['pod.vcpu']}`;
      document.getElementById('pod-vcpu').appendChild(createVariationElement(data.registroA['pct_change_pod_vcpu']));
      
      document.getElementById('pod-memory').textContent = `Memória: ${data.registroA['pod.memory']}`;
      ...
      document.getElementById('oracle-vcpu').textContent = `vCPU: ${data.registroA['oracle.vcpu']}`;
      document.getElementById('oracle-vcpu').appendChild(createVariationElement(data.registroA['pct_change_oracle_vcpu']));
      
      document.getElementById('oracle-memory').textContent = `Memória: ${data.registroA['oracle.memory']}`;
      document.getElementById('oracle-memory').appendChild(createVariationElement(data.registroA['pct_change_oracle_memory']));
      
      document.getElementById('oracle-storage').textContent = `Storage: ${data.registroA['oracle.storage']}`;
      ...
      mssqlBases.textContent = `Bases: ${data.registroA['mssql.bases']}`;
      mssqlBases.appendChild(createVariationElement(data.registroA['pct_change_mssql_bases']));
      
      document.getElementById('mssql-vcpu').textContent = `vCPU: ${data.registroA['mssql.vcpu']}`;
      document.getElementById('mssql-vcpu').appendChild(createVariationElement(data.registroA['pct_change_mssql_vcpu']));
      
      document.getElementById('mssql-memory').textContent = `Memória: ${data.registroA['mssql.memory']}`;
      document.getElementById('mssql-memory').appendChild(createVariationElement(data.registroA['pct_change_mssql_memory']));
      
      document.getElementById('mssql-storage').textContent = `Storage: ${data.registroA['mssql.storage']}`;
      ...
      mongodbBases.textContent = `Bases: ${data.registroA['mongodb.bases']}`;
      mongodbBases.appendChild(createVariationElement(data.registroA['pct_change_mongodb_bases']));
      
      document.getElementById('mongodb-vcpu').textContent = `vCPU: ${data.registroA['mongodb.vcpu']}`;
      document.getElementById('mongodb-vcpu').appendChild(createVariationElement(data.registroA['pct_change_mongodb_vcpu']));
      
      document.getElementById('mongodb-memory').textContent = `Memória: ${data.registroA['mongodb.memory']}`;
      document.getElementById('mongodb-memory').appendChild(createVariationElement(data.registroA['pct_change_mongodb_memory']));
      
      document.getElementById('mongodb-storage').textContent = `Storage: ${data.registroA['mongodb.storage']}`;
      document.getElementById('mongodb-storage').appendChild(createVariationElement(data.registroA['pct_change_mongodb_storage']));
      
      // Projetos
      document.getElementById('project-done').textContent = data.registroA['project.done'];
      document.getElementById('project-done-investment').textContent = formatCurrency(data.registroA['project.done.investment']);
      
      document.getElementById('project-going').textContent = data.registroA['project.going'];
      document.getElementById('project-going-investment').textContent = formatCurrency(data.registroA['project.going.investment']);
      
      document.getElementById('project-new-study').textContent = data.registroA['project.new_study'];
      document.getElementById('project-new-study-investment').textContent = formatCurrency(data.registroA['project.new_study.investment']);
      
      document.getElementById('project-paused').textContent = data.registroA['project.paused'];
      document.getElementById('project-paused-investment').textContent = formatCurrency(data.registroA['project.paused.investment']);
